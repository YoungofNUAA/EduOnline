              ...
              <li v-for="teacher in data.items" :key="teacher.id">
                <section class="i-teach-wrap">
                  <div class="i-teach-pic">
                    <a :href="'/teacher/'+teacher.id" :title="teacher.name" target="_blank">  <!--注意这里1要去掉 否则后端查找不到 teacher=null-->
...
export default {
    asyncData({params,error}){
        ...
        .then(response=>{
          response.data.data  //得到map集合
          return {data:response.data.data}  //data存放map
        })
    },
    methods:{
      gotoPage(page){
